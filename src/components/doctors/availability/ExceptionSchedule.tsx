...
const ExceptionSchedule: React.FC<ExceptionScheduleProps> = ({ exceptions, onChange }) => {
  ...
  const handleAddException = useCallback(() => {
    try {
      ...
      const exceptionDateIST = startOfDay(toIST(selectedDate));
      
      if (exceptionDateIST < todayIST) {
        throw new Error('Cannot add exceptions for past dates');
      }

      // Format date in IST
      const formattedDate = formatISTDate(exceptionDateIST);
      console.log('Adding exception for IST date:', formattedDate);
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div>
            ...
            <DatePicker
              ...
              onChange={(date: Date) => {
                if (date) {
                  // Store date in local timezone, will convert to IST when needed
