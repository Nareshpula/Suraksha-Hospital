-- Update send_sms function with new sender ID and template support
...
BEGIN
  ...
  ELSE
    -- For confirmation messages, use template
    v_request_body := jsonb_build_object(
      'route', 'dlt',
      'sender_id', 'SRKHOP',
      'template_id', '178285',
...
-- Update SMS templates with new template IDs
UPDATE sms_templates 
SET template = CASE 
  WHEN type = 'otp' THEN '{{#var#}}'  -- Template ID: 178284
  WHEN type = 'appointment_confirmation' THEN 'Your appointment with {{doctor_name}} is confirmed for {{date}} at {{time}}. Location: Suraksha Hospital, Madanapalle. For queries: 08571-220345'  -- Template ID: 178285
END,
updated_at = now()
