-- Update send_sms function to use DLT templates
...
BEGIN
  ...
  IF p_type = 'otp' THEN
    -- For OTP messages using template 178284
  ...
  ELSE
    -- For confirmation messages using template 178285
    v_request_body := jsonb_build_object(
      'route', 'dlt',
      'sender_id', 'SRKHOP',
      'message', '178285',
...
-- Update send_appointment_confirmation function to format variables correctly
...
BEGIN
  ...
  v_variables := p_doctor_name || '|' || p_date || '|' || p_time;
...
-- Update SMS templates with DLT template IDs
UPDATE sms_templates 
SET template = CASE 
  WHEN type = 'otp' THEN '178284'
  WHEN type = 'appointment_confirmation' THEN '178285'
END,
updated_at = now()
